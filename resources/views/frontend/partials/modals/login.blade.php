                      ...
                      <form method="POST" action="{{ route('login') }}" class="pt-4">
                        ...
                        <div class="btn-box">
                            ...
                            <div class="text-center">
                                ...
                                <p class="fs-14 pt-2">Don't have an account? <a href="#" class="text-color hover-underline" data-toggle="modal" data-target="#registerModal">Register</a></p>
                            </div>
                        </div><!-- end btn-box -->
                      </form>
...
@push('frontend-addon-script') 
    @if(Session::has('toastr_errors_login'))
        <script>
            const errors = {!! json_encode(Session::get('toastr_errors_login')) !!};
            
            // Ambil field-field yang error karena "required"
            const requiredFields = [];
            const messages = [];

            errors.forEach(msg => {
                if (msg.toLowerCase().includes('required')) {
                    if (msg.toLowerCase().includes('email')) requiredFields.push('Email');
                    if (msg.toLowerCase().includes('password')) requiredFields.push('Password');
                } else {
                    messages.push(msg); // pesan selain 'required'
                }
            });

            // Gabungkan pesan required
            if (requiredFields.length > 0) {
                const fieldStr = requiredFields.join(', ');
                messages.unshift(`${fieldStr} ${requiredFields.length > 1 ? 'are' : 'is'} required`);
            }
